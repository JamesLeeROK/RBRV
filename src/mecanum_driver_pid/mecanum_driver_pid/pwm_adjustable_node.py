...
class PIDController:
    ...
    # 💡 가변 최대 출력을 인자(dynamic_max_out)로 받도록 수정
    def compute(self, target, actual, dt, dynamic_max_out):
        error = target - actual
        self.integral += error * dt
        # 적분 와인드업 방지도 가변 한계치에 맞춤
...
class MecanumDriverNode(Node):
    def __init__(self):
        super().__init__('pwm_adjustable_node')
        ...
        # 💡 [핵심] 모션별 독립적인 가변 출력 제한 파라미터 도입
        self.declare_parameter('max_pwm_fwd', 35)      # 직진/후진 (슬립 방지용 낮은 출력)
        self.declare_parameter('max_pwm_strafe', 60)   # 좌우 평행이동 (마찰 극복용 높은 출력)
        self.declare_parameter('max_pwm_turn', 65)     # 제자리 돌기 (강한 토크 필요)
        ...
        self.get_logger().info("메카넘 모션 인지형 가변 출력 드라이버 시작!")
    ...
    def control_loop(self):
        ...
        # 💡 현재 명령의 종류를 파악하여 그에 맞는 MAX_PWM 한계치를 실시간으로 가져옵니다.
        if abs(wz) > 0.01:
        ...
        if vx == 0.0 and vy == 0.0 and wz == 0.0:
            ...
            for i in range(4):
                # 💡 실시간으로 변하는 current_max_pwm 한계치를 PID에 넘겨줍니다.
